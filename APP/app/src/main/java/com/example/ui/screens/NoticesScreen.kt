...
fun NoticesScreen(
    ...
    val categories = remember(notices) {
        listOf("All") + notices.map { it.category }.distinct()
    }
    ...
    Column(modifier = modifier.fillMaxSize()) {
        ...
        Row(
            ...
            Text(
                text = "Official Announcements (${filteredNotices.size})",
        ...
        if (filteredNotices.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(filteredNotices, key = { it.id.ifBlank { it.title } }) { notice ->
                    NoticeCard(
                        notice = notice,
                        onViewDocument = {
                            IntentHelpers.viewPdfDocument(context, it.documentUrl, it.title)
                        },
                        onDownloadDocument = {
                            IntentHelpers.downloadPdfDocument(context, it.documentUrl, it.title)
                        }
                    )
                }
            }
